...
int Renderer::_render( IsometricTile* tile, Tileset::Visibility vis, int off_x, int off_y )
{
	const int offset = 16;
	...
	int x_coeff = -2 * offset + off_x;
	int y_coeff = -offset + off_y;
    ...
    for( int iWidth = map_size.x - 1; iWidth >= 0; --iWidth )
    {
        for( int iLength = map_size.y - 1; iLength >= 0; --iLength )
        {
            for( int iHeight = 0; iHeight < map_size.z; ++iHeight )
            {
                ...
                _render( main_tile, vis, off_x, off_y );
            }
